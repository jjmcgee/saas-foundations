    ...
    def handle(self, *args: Any, **options: Any):
        ...
        for name, url in VENDOR_STATICFILES.items():
            ...
            if dl_success:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Failed to download {url}') # pylint: disable=E1101
                )
        if set(completed_urls) == set(VENDOR_STATICFILES.values()):
            self.stdout.write(
                self.style.SUCCESS('Successfully updated all vendor static files.') # pylint: disable=E1101
            )
        else:
            self.stdout.write(
                self.style.WARNING('Some files were not updated.') # pylint: disable=E1101
            )
